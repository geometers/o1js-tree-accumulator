...
function createDerivers() {
  ...
  function provable3(typeObj, options) {
    ...
    let { empty: empty4, fromJSON, toJSON, toInput } = signable2(typeObj, isProvable);
    return {
...
function witness(type, compute) {
  ...
  try {
    fields2 = exists(type.sizeInFields(), () => {
      proverValue = type.fromValue(compute());
      let fields3 = type.toFields(proverValue);
  ...
  let value = type.fromFields(fields2, aux);
  type.check(value);
  return value;
}
async function witnessAsync(type, compute) {
  ...
  if (!inCheckedComputation() || ctx.inWitnessBlock) {
    ...
    return clone(type, value2);
  }
  let proverValue = void 0;
  let fields2;
  let id2 = snarkContext.enter({ ...ctx, inWitnessBlock: true });
  try {
    fields2 = await existsAsync(type.sizeInFields(), async () => {
...
`);
  ...
  static witness(compute) {
    return witness(_Unconstrained.provable, () => new _Unconstrained(true, compute()));
  }
  /**
...
function fieldToField3(x) {
  ...
  let xBig = witness(Field32.provable, () => Field32.from(x.toBigInt()));
  ...
  let proofsToVerify = proofArgs.map((Proof3) => {
    ...
    if (tag === proofSystemTag)
      ...
      if (SideloadedTag.get(tag.name) === void 0) {
        computedTag = Pickles.sideLoaded.create(tag.name, Proof3.maxProofsVerified, Proof3.publicInputType?.sizeInFields() ?? 0, Proof3.publicOutputType?.sizeInFields() ?? 0);
...
var Packed = class _Packed {
  ...
  static create(type) {
    ...
    return _a = class Packed_ extends _Packed {
      static empty() {
    ...
    }, _a._innerProvable = type, _a._provable = provableFromClass(_a, {
...
var Hashed = class _Hashed {
  ...
  static create(type, hash3) {
    var _a;
    let _hash = hash3 ?? ((t) => Poseidon2.hashPacked(type, t));
    return _a = class Hashed_ extends _Hashed {
      static empty() {
        let empty4 = type.empty();
...
function negateIf(condition, P, f) {
  let y = Provable.if(Bool3.Unsafe.fromField(condition), Field32.provable, ForeignField.negate(P.y, f), P.y);
  return { x: P.x, y };
}
...
function decomposeNoRangeCheck(Curve, s) {
  ...
  let lambda = Provable.if(Bool3.Unsafe.fromField(s1Negative), Field32.provable, Field32.from(Curve.Scalar.negate(Curve.Endo.scalar)), Field32.from(Curve.Endo.scalar));
  let rhs = Provable.if(Bool3.Unsafe.fromField(s0Negative), Field32.provable, ForeignField.Sum(s).add(s0).finish(Curve.order), ForeignField.Sum(s).sub(s0).finish(Curve.order));
...
function MerkleListBase() {
  return class extends Struct({ hash: Field4, data: Unconstrained.provable }) {
    static empty() {
  ...
  static create(type, nextHash = merkleListHash(type), emptyHash_ = emptyHash) {
    class MerkleListTBase extends _MerkleList {
      ...
      static from(array) {
        ...
        let unconstrained = Unconstrained.witness(() => data.map((x) => toConstant2(type, x)));
...
var MerkleListIterator = class _MerkleListIterator {
  ...
  static create(type, nextHash = merkleListHash(type), emptyHash_ = emptyHash) {
    var _a;
    return _a = class Iterator extends _MerkleListIterator {
      static from(array) {
        let { hash: hash3, data } = withHashes(array, nextHash, emptyHash_);
        let unconstrained = Unconstrained.witness(() => data.map((x) => toConstant2(type, x)));
    ...
    }), _a._nextHash = nextHash, _a._emptyHash = emptyHash_, _a;
  }
  ...
  let obj = {
    ...
    requireNothing() {
      ...
      if ("isSome" in property) {
        ...
        if ("lower" in property.value && "upper" in property.value) {
          if (fieldType === UInt642) {
            property.value.lower = UInt642.zero;
            property.value.upper = UInt642.MAXINT();
          } else if (fieldType === UInt322) {
            property.value.lower = UInt322.zero;
            property.value.upper = UInt322.MAXINT();
          }
        } else {
...
var AccountUpdateForest = class _AccountUpdateForest extends MerkleList.create(AccountUpdateTreeBase, merkleListHash2) {
  ...
  static fromSimpleForest(simpleForest) {
    let nodes = simpleForest.map((node) => {
      ...
      let children = _AccountUpdateForest.fromSimpleForest(node.children);
      return { accountUpdate, children, id: node.accountUpdate.id };
    });
    return _AccountUpdateForest.fromReverse(nodes);
  }
  ...
  static assertConstant(forest) {
    Provable.asProver(() => {
      forest.data.get().forEach(({ element: tree }) => {
        ...
        _AccountUpdateForest.assertConstant(tree.children);
      });
    });
  }
};
...
var UnfinishedForest = class _UnfinishedForest {
  ...
  witnessHash() {
    let final = Provable.witness(AccountUpdateForest.provable, () => this.finalize());
  ...
  return {
    ...
    set(state3) {
      ...
      stateAsFields.forEach((x, i) => {
        ...
        if (!appStateSlot)
          throw Error(`Attempted to set on-chain state variable \`${this._contract?.key}\`. Currently, only a total of 8 fields elements of on-chain state are supported.`);
    ...
    requireEquals(state3) {
      ...
      let accountUpdate = this._contract.instance.self;
      stateAsFields.forEach((x, i) => {
        AccountUpdate3.assertEquals(accountUpdate.body.preconditions.account.state[layout.offset + i], x);
      });
  ...
  Use the optional \`maxUpdatesWithActions\` argument to increase this number.`);
    ...
    getActions(config) {
      ...
      let actions = Provable.witness(MerkleActions.provable, () => {
        let actionFields = getActions(contract.address, config, contract.tokenId);
        let actions2 = actionFields.map((event) => event.actions.map((action) => reducer.actionType.fromFields(action.map(Field4))));
        return MerkleActions.from(actions2.map((a2) => ActionList2.fromReverse(a2)));
      });
...
method.returns = function(returnType) {
  return function decorateMethod(target, methodName, descriptor) {
    return method(target, methodName, descriptor, returnType);
  };
};
...
var AccountUpdateIterator = MerkleListIterator.createFromList(AccountUpdateForest);
...
var MayUseToken = AccountUpdate3.MayUseToken;
var TokenAccountUpdateIterator = class _TokenAccountUpdateIterator {
  ...
  static create(forest, selfToken) {
    return new _TokenAccountUpdateIterator(AccountUpdateIterator.startIteratingFromLast(forest), MayUseToken.ParentsOwnToken, selfToken);
  }
  /**
...
var TokenContract = class extends SmartContract {
  ...
  forEachUpdate(updates, callback) {
    ...
    iterator.assertFinished(`Number of account updates to approve exceed the supported limit of ${MAX_ACCOUNT_UPDATES}.
`);
...
var MerkleLeaf = class _MerkleLeaf extends Struct({
  ...
  prefix: Unconstrained.provableWithEmpty([])
}) {
...
function sliceActions(actions, batchSize) {
  class ActionListsList extends MerkleList.create(ActionList.provable, (hash3, actions2) => Actions.updateSequenceState(hash3, actions2.hash), actions.currentHash) {
  }
...
function OffchainState(config, options) {
  ...
  function contract() {
    ...
    assert3(ctx.this.constructor === internal.contract.constructor, "Offchain state methods can only be called on the same contract that you called setContractInstance() on");
    return ctx.this;
  }
  ...
  return {
    ...
    async settle(proof) {
      proof.verify();
      let state3 = contract().offchainState.getAndRequireEquals();
